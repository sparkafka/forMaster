...
public class CustomDeserializeTest {
    ...
    public static void main(String[] args) {
        ...
        prop2.setProperty("value.deserializer", "org.example.PersonDeserializer");

        Consumer<String, Person> consumer = new KafkaConsumer<>(prop2);
        consumer.subscribe(Collections.singletonList(topic));
        System.out.println("Start");
        for (int i=1;i<=10000;i++){
            ConsumerRecords<String, Person> records = consumer.poll(Duration.ofMillis(10));
            for (ConsumerRecord<String, Person> record : records) {
                System.out.println("key: " + record.key() + " name: " + record.value().getName() + " age: " + record.value().getAge());
